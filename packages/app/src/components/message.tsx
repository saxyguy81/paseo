...
import { useTurnCompletionTime } from "@/hooks/use-turn-completion-time";
 ...
 * Shows both how long the turn ran and when it ended. Relative timestamps use
 * the shared clock, so a long history does not create one timer per footer.
 */
export const AssistantTurnFooter = memo(function AssistantTurnFooter({
  ...
  const timestampLabel = useTurnCompletionTime(completedAt ?? null);
  const statusLabel = [durationLabel, timestampLabel ? `ended ${timestampLabel}` : ""]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    <View style={assistantTurnFooterStylesheet.container}>
      ...
      {statusLabel ? <Text style={assistantTurnFooterStylesheet.label}>{statusLabel}</Text> : null}
    </View>
  );
});
